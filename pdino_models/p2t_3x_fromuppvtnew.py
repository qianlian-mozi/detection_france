...
class SpatialReductionAttention(MultiheadAttention):
    ...
    def __init__(self,
        ...
        self.q = nn.Sequential(nn.Linear(embed_dims, embed_dims, bias=qkv_bias))
        self.kv = nn.Sequential(nn.Linear(embed_dims, embed_dims * 2, bias=qkv_bias))  
    ...
    def forward(self, x, hw_shape, identity=None):
        ...
        x_q = q
        for (pool_ratio,c) in zip(self.pool_ratios,self.conv):  
            # pool_ratio = sum(pool_ratio) # 增加了第二个zip对象后竟然就自动成int了，就不需要这句话了
            pool = F.adaptive_avg_pool2d(x_, (round(H/pool_ratio), round(W/pool_ratio)))
            #     pool = F.adaptive_avg_pool2d(x_, (round(H/pool_ratio), round(W/pool_ratio)))
            #     TypeError: unsupported operand type(s) for /: 'int' and 'tuple',意思是poolratio不应该在这里解包后还是tuple 2023.12.8
            pool = pool + c(pool) # 报错notimplement，眸佬的建议是检查conv的类，如果是modulelist是不能这么直接作为conv使用的
        ...
        if self.sr_ratio >= 1:
            ...
            out = (attn @ v)
            out = x.transpose(1,2).contiguous().reshape(B,N,C)
            # x = self.proj(out)
        else:
            x_kv = x
...
class PyramidVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        #     [nn.Conv2d(in_channels, embed_dims_i[0], kernel_size=4, stride=4, padding=0, groups=1) for
        #      temp
        #      in pool_ratios[0]])
        # self.d_convs2 = nn.ModuleList(
        #     [nn.Conv2d(embed_dims_i[0], embed_dims_i[1], kernel_size=2, stride=2, padding=0, groups=1) for
        #      temp
        #      in pool_ratios[1]])
        # self.d_convs3 = nn.ModuleList(
        #     [nn.Conv2d(embed_dims_i[1], embed_dims_i[2], kernel_size=2, stride=2, padding=0, groups=1) for
        #      temp
        #      in pool_ratios[2]])
        # self.d_convs4 = nn.ModuleList(
        #     [nn.Conv2d(embed_dims_i[2], embed_dims_i[3], kernel_size=2, stride=2, padding=0, groups=1) for
        #      temp
        #      in pool_ratios[3]])
        # identity = [self.d_convs1, self.d_convs2, self.d_convs3, self.d_convs4]
        ...
        for i, num_layer in enumerate(num_layers):
            embed_dims_i = embed_dims * num_heads[i]  # num_heads是跟随stage变化的，range（3），所以这个i标识符可以安心用
            ...
            # if i == 0:
            #     dconv = nn.ModuleList(
            #     [nn.Conv2d(in_channels, embed_dims_i, kernel_size=4, stride=4, padding=0, groups=1) for
            #      temp
            #      in pool_ratios[i]])
            # else:
            #     embed_dims_i_1 = embed_dims * num_heads[i-1]
            #     dconv = nn.ModuleList(
            #     [nn.Conv2d(embed_dims_i_1, embed_dims_i, kernel_size=2, stride=2, padding=0, groups=1) for
            #      temp
            #      in pool_ratios[i]])
            
           
            dconv = nn.ModuleList(
            [nn.Conv2d(embed_dims_i, embed_dims_i, kernel_size=3, stride=1, padding=1, groups=embed_dims_i) for
                temp
                in pool_ratios[i]])
...
@MODELS.register_module()
class p2t_tiny(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super().__init__(
            patch_size=[4,4,4,4], embed_dims=48, num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
            qkv_bias=True,  depths=[2, 2, 6, 3], norm_after_stage=True,use_abs_pos_embed=False,use_conv_ffn=True,
